...
namespace ConsoleMenuToolKit.Application
{
    ...
    public sealed class ConsoleMenuSetup
    {
        ...
        {
            var console = new ConsoleMenuWrapper();

            _selector = new ConsoleMenuSelector(console);
            _executor = new ConsoleMenuExecutor([], [], console);
        ...
        {
            if (!System.Enum.IsDefined(typeof(ConsoleMenuSelectionType), selectionType))
            {
        ...
        private void EnsureUniqueId(int id) 
        {
            if (_options.Any(x => x.Id == id))
                throw new InvalidOperationException(
                    $"An option with ID '{id}' already exists.");
        }
    }
}
